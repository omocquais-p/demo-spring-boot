...
ytt -f "$SCRIPT_DIR"/ootb-supply-chain-testing-template.yaml --data-value registry.server="$TAP_REGISTRY_SERVER" --data-value registry.repository="$TAP_REPOSITORY" > "$SCRIPT_DIR"/ootb-supply-chain-testing-values.yaml
cat "$SCRIPT_DIR"/ootb-supply-chain-testing-values.yaml
